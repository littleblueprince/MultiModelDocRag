...
from util.chat_template import get_temp_messages_v3
import aiohttp
...
def query_rewrite(original_question: str, sub_question: str, contexts: list[str],
    ...
    instruction = """
    **task:**
    Rewrite the current sub-question to make it more precise based on the provided context.
...
store_path = '/data1/zch/MultiModelDocRag/generate_augment/dag/prediction_qwen2_vl_7b_dev_v4.jsonl'
...
for item in tqdm(data):
    ...
    for node in graph:
        ...
        if not dependencies:  # 不存在需要解决的依赖
            ...
            messages = get_temp_messages_v3(question=query, corpus=corpus)
            node['answer'] = llm_predict(messages=messages)
        else:  # 存在依赖未解决
