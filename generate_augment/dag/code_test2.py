...
from util.chat_template import get_normal_QA_prompt_v1
...
# model_name = "/mnt/sdb/zch/models/Qwen2-VL-7B-Instruct"
# base_url = "http://36.213.0.171:8001/v1"

api_key = 'test'
QA_client = OpenAI(api_key=api_key, base_url=base_url)
QW_client = instructor.from_openai(OpenAI(api_key=api_key, base_url="http://36.137.79.97:30250/v1"))


def llm_predict(messages: list, model_name='Qwen2-VL-7B-Instruct') -> str:
    chat_completion = QA_client.chat.completions.create(
        messages=messages,
        model=model_name,
    )
...
def query_rewrite(original_question: str, sub_question: str, contexts: list[str],
                  model_name='qwen2-72b-instruct') -> IntermediateAnswer:
    instruction = """
**Task:**  
Assess if the provided context is sufficient to directly address the original question. If sufficient, provide a clear answer along with its justification and indicate the output type as "answer". If not, refine the subquery based on the context, integrate any relevant subqueries to enhance reasoning, and indicate the output type as "subquery".  
...
```json
{{
  "type": "answer" or "subquery",
  "content": "<your output here>"
}}
```  
    """
    # chat_completion = QW_client.chat.completions.create(
    res = QW_client.chat.completions.create(
        ...
        model=model_name,
        response_model=IntermediateAnswer,
    )
    # return chat_completion.choices[0].message.content
    return res
...
for item in tqdm(data):
    ...
    for node in graph:
        ...
        if not dependencies:  # 不存在需要解决的依赖
            ...
            messages = get_normal_QA_prompt_v1(original_question=question, sub_question=query, corpus=corpus)
            ...
            res = query_rewrite(question, query, contexts=node['contexts'])
            if res.type == 'answer':
                node['answer'] = res.content
                break
            else:
                node['rewritten_query'] = res.content
            node['contexts'] = []
