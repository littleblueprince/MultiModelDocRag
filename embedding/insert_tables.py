...
def get_text_embedding(text: str, url: str = "http://36.213.0.171:8003/encode-text/"):
    data = {"text": text}
    try:
...
def get_img_embedding(text: str, url: str = "http://36.213.0.171:8003/encode-image/"):
    data = {"text": text}
    try:
        response = requests.post(url, json=data)
        if response.status_code == 200:
...
if __name__ == "__main__":
    ...
    inserted_ids_file = "inserted_tabs_ids.txt"
    # 加载已插入的ID
    ...
    tables_path = '/data1/zch/datasets/multimodalqa/MMQA_tables_md.jsonl'
    tables_dataset = read_jsonl(tables_path)
    batched_data = chunked(tables_dataset, 512)
    for batch in tqdm(batched_data):
        ...
        for item in batch:
            ...
            table = item['table']
            id = item['id']
            vector = get_text_embedding(f'**table title**:{title}**table content**:{table}')
            insert_datas.append(
                {
                    "content": f'**table title**:{title}**table content**:{table}',
                    "vector": vector,
                    "metadata": item
                }
            )
