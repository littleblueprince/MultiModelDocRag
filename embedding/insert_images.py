...
async def async_process_item(item):
    title = item['title']
    path = item['path']
    dir = '/mnt/xlj/rag paper/dataset/multimodelQA/final_dataset_images/'
    # 归一化之后的结果导入
    # dir = '/mnt/sdb/zch/LLaMA-Factory/data/qwen2_vl_ft/mllm_data/'
    # base = os.path.splitext(path)[0]
    # path = base + ".png"
    try:
        vector = await get_embedding_async(text=title, file_path=dir + path)
    ...
    return {
        "content": f'img_title:{title} img_path:{path}',
        "vector": vector,
        "metadata": item
    }
...
async def main():
    inserted_ids_file = "inserted_imgs_ids_v2.txt"
    ...
    imgs_dataset_filtered = [item for item in imgs_dataset if item['id'] not in inserted_ids]

    print(f'{len(imgs_dataset_filtered)} images need to be inserted')

    # 处理所有图片，并将它们插入到 Milvus
    total_inserted = await process_all_items(imgs_dataset_filtered, milvus, collection_name, inserted_ids_file,
                                             batch_size=32)
